...
const ContentForCustomerRoot = styled(`div`)`
  ...
  return (
    <UserContext.Consumer>
      {({ customer }) => {
        const {
          shopify: { codes },
          github: { customerCount }
        } = customer;

        const showLevelTwoIncentive = customerCount >= 1 && customerCount < 5;
        let customersToGo, percentToGo;
        if (showLevelTwoIncentive) {
          customersToGo = 5 - customerCount;
          percentToGo = ((5 - customersToGo) / 5) * 100;
        }

        const numberOfCodes = codes.filter(code => code.used === false).length;
        let text;
        if (numberOfCodes > 1) {
          text = `Use these discount codes during checkout to claim some free craft!`;
        } else if (numberOfCodes == 1) {
          text = `Enter this discount code during checkout to claim your free craft!`;
        } else {
          text = `Looks like you've claimed your craft! Thanks again, and keep being awesome.`;
        }

        return (
          <ContentForCustomerRoot>
            <Heading>Here you go!</Heading>
            <Text>
              Thanks for going the extra mile to help build Gatsby! 💪 You have
              made <strong>{customerCount}</strong>{' '}
            ...
            {showLevelTwoIncentive && (
              <>
                <CodeBadgeBox key={`SHOPPINGSPREE`}>
                  <CodeBadge>
                    <Name code={`SHOPPINGSPREE`}>{`Level 2 Swag Code`}</Name>
                    <Code>
                      <LockIcon />
                    </Code>
                  </CodeBadge>
                ...
                <Text>{`Make ${customersToGo} more customer${
                  customersToGo > 1 ? `s` : ``
                } to earn level 2 craft!`}</Text>
              </>
            )}
          </ContentForCustomerRoot>
        );
...
export default ContentForCustomer;
